(function () {
  ...
  const FabDashboard = ({ navigate }) => {
    ...
    const activity = uM(() => {
      // The list endpoint does not return approval_logs, so r.history is always [].
      // Derive one activity event per request from rawStatus + updated timestamp instead.
      const items = [];
      requests.forEach(r => {
      ...
      >
        ...
        <FabCard padding={0} style={{ overflow: 'hidden' }}>
          ...
          <div style={{ padding: '24px 28px 26px' }}>
            {(() => {
              const STYLES = {
                APPROVE:   { dot: '#1f8a5b', tintBg: '#e8f6ee', tintFg: '#157a4a', verb: 'Approved',   icon: (c) => <F.Check size={12} color={c} strokeWidth={3} />,      text: (a) => <>{a.request.title} approved — awaiting sample shipment</> },
                SHIP:      { dot: '#2980b9', tintBg: '#ddeef8', tintFg: '#1a6696', verb: 'Shipped',    icon: (c) => <F.Package size={11} color={c} strokeWidth={2.5} />,    text: (a) => <>{a.request.title} — samples shipped to lab</> },
                IN_PROG:   { dot: '#6c67b8', tintBg: '#ecebf7', tintFg: '#5550a0', verb: 'In Progress',icon: (c) => <F.Activity size={11} color={c} strokeWidth={2.5} />,  text: (a) => <>{a.request.title} — processing in progress</> },
                COMPLETED: { dot: '#0f766e', tintBg: '#ccfbf1', tintFg: '#0f766e', verb: 'Completed',  icon: (c) => <F.CircleCheck size={11} color={c} strokeWidth={2.5} />,text: (a) => <>{a.request.title} — experiment completed</> },
